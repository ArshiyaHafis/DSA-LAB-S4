...
struct bst *find(struct bst *temp, int k)
{
    if (temp == NULL)
    {
        return NULL;
    ...
    struct bst *temp1 = find(temp->left, k);
    if (temp1 != NULL)
    {
        return temp1;
    }
    struct bst *temp2 = find(temp->right, k);
    if (temp2 != NULL)
    {
        return temp2;
    }
...
void descendants(struct bst *node, int k)
{
    if (node == NULL || k < 0)
    {
        return;
    ...
    descendants(node->left, k - 1);
    descendants(node->right, k - 1);
}
int main()
{
    struct bstroot *T, tt;
